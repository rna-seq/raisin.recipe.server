...
test: bin/nosetests
	bin/nosetests -s raisin/recipe/server

coverage: bin/coverage bin/nosetests
	bin/nosetests --with-coverage --cover-html --cover-html-dir=html --cover-package=raisin.recipe.server
	bin/coverage html

pylint:	bin/pylint
	bin/pylint -i y raisin/recipe/server

flake8:	bin/flake8
	bin/flake8 --max-complexity 12 raisin/recipe/server

pep8:	bin/pep8
	bin/pep8 raisin/recipe/server

pyflakes:	bin/pyflakes
	bin/pyflakes raisin/recipe/server
...
templer: bin/python
	...
	export OLDHOME="${HOME}"; export HOME="${PWD}"; ./bin/templer dotdotpackage raisin.recipe.server; export HOME="${OLDHOME}"

diff: bin/python
	# Show the difference between the current package and the regenerated one
	colordiff -c -r raisin.recipe.server .|less -r

sloccount:	bin/python
	sloccount raisin/recipe/server
